...
-- 결과셋 9개: ①대회 → ②순위표 → ③경기 → ④수상 → ⑤역대 우승(같은 SeriesSlug의 타 연도 Champion)
--            → ⑥영상 → ⑦뉴스 → ⑧등장 팀의 공개 슬러그(팀명→팀 홈 링크용, 공개 팀만)
--            → ⑨상세 보유 경기 MatchId(이벤트/출전 존재 — 행 확장 셰브론용). MultiQueryReader로 소비.
...
BEGIN
    ...
    SELECT
        ...
        m.[Status], m.[MatchedAt], m.[VenueName], m.[MatchType], m.[MatchSequence],
        m.[DataSource], m.[ExternalId], m.[SyncStatus],
    ...
    -- ⑨ 상세 보유 경기 (이벤트 또는 출전 기록이 하나라도 있으면 셰브론 노출 대상)
    SELECT m.[MatchId]
    FROM [dbo].[SoccerMatches] m WITH (NOLOCK)
    WHERE m.[TournamentId] = @TournamentId AND m.[DeletedAt] IS NULL
      AND (EXISTS (SELECT 1 FROM [dbo].[SoccerMatchEvents] e WITH (NOLOCK) WHERE e.[MatchId] = m.[MatchId] AND e.[DeletedAt] IS NULL)
        OR EXISTS (SELECT 1 FROM [dbo].[SoccerMatchAppearances] ap WITH (NOLOCK) WHERE ap.[MatchId] = m.[MatchId] AND ap.[DeletedAt] IS NULL));
END

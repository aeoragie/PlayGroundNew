...
- **컬럼·파라미터의 NULL 허용 여부가 곧 C# 널 허용 여부다.** 제너레이터가 `NULL`이면
  `string?`·`int?`로 생성한다(`NOT NULL`이면 널 비허용). 그러니 **SQL에서 NULL 허용을 잘못 적으면
  C# 쪽 널 검사가 통째로 어긋난다** — 스키마를 진실로 삼고 여기서 정확히 적는다.

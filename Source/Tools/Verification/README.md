...
| `api-roster.js` | 로스터 쓰기 API — 추가(코드 발급)·등번호/연령대 검증·내보내기·복구·남의 팀 거부 |
| `shot-roster.js` | 로스터 쓰기 UI — ＋선수 추가→토스트→목록 반영 / ⋯→내보내기→확인 모달→실행취소 |
| `sql-roster-cleanup.sql` | 검증 선수 물리 삭제 (스크립트는 소프트 삭제만 남긴다 — 끝나고 실행) |
...
| `api-teamboard.js` | 팀 게시판 API(Design.TeamBoard) — 공지 발행→**보호자 TeamNotice 알림(전원)**→공개 전환→**소개 탭 소식 반영(FileUrl·유형 미노출)**→보호자 뷰 안읽음→읽음 처리 · 자료는 알림 없음 · **고정 최대 2**(3번째 거부) · 입력 검증(제목/본문/유형/외부 URL) · 남의 팀 삭제 거부 · 삭제→소식에서도 사라짐→복구 · 남의 자녀 보호자 뷰 빈 결과. 관리자=verify-u15-1 / 보호자=verify-player-u15. 끝나면 `LEFT(Title,4)='[TB]'` 물리 삭제(알림·읽음 포함)로 원복 |
| `shot-teamboard.js` | 팀 게시판 UI — 관리자 대시보드 게시판(세그먼트 전체/공지/자료·**고정 네이비 틴트 행**·눈 아이콘(공개 중 teal)·⋯·＋글 작성) PC/모바일 · 작성 다이얼로그(유형·공개 스위치 문구) · **공개홈 소개 탭 "팀 소식"**(공개 공지 노출·비공개 자료 미노출) · 보호자 뷰(**안읽음 오렌지 점** `#FF6B35`·상세 열람) · 모바일 가로 스크롤 없음. 스크린샷 6종. 끝나면 생성 글 삭제(API 소프트 + SQL 물리) |
| `api-notifications.js` | 알림 센터 페이지 API(DECISION.NOTIFICATIONCENTER) — **90일 정리**(100일 시드 삭제→25건) · 세그먼트 필터(all/action/unread) 카운트 3종 · **더 보기 20**(offset 20→나머지) · 처리 필요=액션형 라이브 Pending · **bulk 읽음→unread 0·벨 0**·읽어도 total/action 불변 · 알 수 없는 필터→all 폴백. 계정 verify-player-u15. 끝나면 시드 알림·요청 삭제로 원복 |
| `api-ogmeta.js` | 링크 공유 미리보기(OG, DECISION.OGMETA) — 크롤러 UA(facebook·kakaotalk)로 **4종 라우트 메타**(랜딩·팀·선수·대회) + **화이트리스트 밖/없는 선수 → 랜딩 폴백** + **사람 UA=SPA(og 없음)** + **PNG 시그니처·1200×630** 3종 + `.png` 요청 비가로채기. 선수=이름+고정문구+브랜드 이미지(개인 이미지 아님). 조회만(데이터 무변경). 시드 식별자는 `OG_TEAM`/`OG_TOUR`/`OG_PLAYER` 환경변수로 갱신 가능. **비공개 게이팅 실측은 SoccerPlayerFieldVisibilities Profile 임시 토글 후 원복** |
| `shot-notifications.js` | 알림 센터 페이지 UI — **비로그인 딥링크→/login?returnUrl**(notifications 보존) · 헤더/세그먼트/90일 캡션/더 보기 · **안읽음 오렌지 점**(첫 로드 유지) · 액션형 승인/거절 버튼 · **세그먼트 URL 왕복**(`?filter=action` 새로고침 유지·전체 클릭 시 쿼리 제거) · 더 보기 행 증가 · **패널 "전체 보기"→/notifications**. 시드 필요(보호자 24건). 스크린샷 2종. 끝나면 SQL로 원복. **함정: 한글 뒤 `@메서드`는 Razor 이메일 리터럴 오인 · Client 수정 후 서버 완전 재기동(낡은 WASM)** |

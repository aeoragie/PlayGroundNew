...
│   │
│   └── Database/
│       └── Main/                      SQL 원본 (버전 관리의 단일 진실 소스)
│           ├── Schema/ Tables/ Procedures/ Queries/ Indexes/
│
└── Tests/
...
- **규칙**: 테이블명 PascalCase 복수형(`Players`), 컬럼명 PascalCase(`PlayerId`),
  프로시저 `Usp` 접두사. 스키마 변경은 반드시 SQL 파일 먼저 수정.
...
- EF Core = CRUD·마이그레이션, Dapper = SP 호출·성능 중요 조회.
- 새 SP 추가 = SQL 파일(`Source/Database/Main/Procedures/`) + C# 호출 클래스 + DB 배포 3단계.

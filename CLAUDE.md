...
2. **Phase B — 입력 UI** (A1·A2 위에): ~~B1 경기 결과 입력(+DatePicker)~~ 완료(아래, **친선경기
   전용으로 용도 축소** — 설계 결정 7) → ~~B2 팀 정보 수정(+ImageUploader)~~ 완료(아래) →
   ~~B4 선수 사진 업로드~~ 완료(아래) → **B5 친선경기 구분** → **B3 커리어·포트폴리오 입력**.
   - **B5를 B3보다 먼저 한다** — 설계 결정 7의 미정리 상태(B1 절 "정리 필요")를 실제로 걷어내는
     작업이라, 오래 둘수록 대회·리그 선택이 남은 입력 화면이 실사용자에게 노출될 위험이 커진다.
...
6. **Phase D — 잔여 패턴**: 별도 단계 없이 화면 작업에 얹는다 (AvatarBadge만 Phase C 후 일괄 교체 1회).
   그 외 잔여: 온보딩 중복 방지. **DropdownMenu는 B6가 선행 소비**하므로 그때 먼저 만든다.

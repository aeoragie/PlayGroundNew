...
## 현재 상태 · 다음 작업 (2026-08-09 갱신)

**첫 운영 배포 완료 — `https://playgroundsport.com` 라이브** (2026-08-09, HTTPS 포함).
당분간 **수동 배포**(publish → tar zip → scp → `playground-deploy`, GitHub Actions 전환은
숙달 후), 백업 크론은 실데이터 유입 전 필수 전환 — 방침·서버 현황은 `Deploy/Servers.md`,
단계 상태는 `Docs/Development/ReleasePlan.md` R4.
...
> 온디맨드로 생성한다 — **실데이터 대량 시드는 `Source/Tools/KfaSeeder`**(KFA 크롤링 JSON →
> 대회 152·경기 4,608·이벤트·라인업 11만 행. **생성물이 `Seeds/Kfa/`에 커밋돼 있어**
> 다른 PC는 적용만 하면 된다. 07이 검증fc를 **플레이그라운드FC**로 개명하고
> 울산HDFCU12 데이터를 연결, 08·09가 테스트 계정 10종을 만든다).
> 경기 상세 확인용 소형 시드 = `Seeds/Verification/OnDemandMatchDetail.Seed.sql`
...
- **배포 이후 기능 아이디어는 `Docs/Development/FeatureBacklog.md`** (2026-08-09) — 핸드폰 인증·
  문자, 푸시 알림, 결제, 유튜브·인스타 콘텐츠 연동 등. 기록만, 착수 시 재검토.
...
- `mLogger.Info("Team created", ("TeamId", id))` — Trace/Debug/Info/Warn/Error/Fatal 제공.
  ...
  `KeyValueLogExtensions.Info(logger, …)` 정적 호출로 쓴다. 진단 로그는 `DiagnosticLog`

...
## 폴더 구조 (FHS) — 자리가 파일의 "종류"로 정해져 있다

### 개념 — Windows와 뭐가 다른가

Windows는 **프로그램별**로 묶는다 — `C:\Program Files\앱이름\` 안에 실행 파일·설정·데이터가
같이 산다. 리눅스는 반대로 **파일의 종류별**로 묶는다 — 한 프로그램의 실행 파일은 `/usr/bin`,
설정은 `/etc`, 로그는 `/var/log`에 흩어져 산다. 이 관례가 FHS(Filesystem Hierarchy Standard)다.

처음엔 불편해 보이지만 강력한 이점이 있다. **어떤 서버에 들어가도 찾는 법이 같다** —
"nginx 설정이 어디 있지?"의 답이 배포판·서버가 달라도 늘 `/etc/nginx/`다. 그리고 백업은
`/etc`(설정)와 `/var`(데이터)만 뜨면 되고, `/usr`는 재설치하면 그만이다 — 종류별로 나뉘어
있어서 가능한 전략이다.
...
| 폴더 | 이름의 유래 | 역할 |
|---|---|---|
| `/etc` | editable text config | **설정 파일**. 프로그램 자체가 아니라 프로그램을 "어떻게 돌릴지" |
| `/usr` | unix system resources | **프로그램과 그 재료**. 패키지 관리자(apt)가 설치하는 실행 파일(`/usr/bin`)·라이브러리(`/usr/lib`) — 평소엔 읽기만 하는 영역 |
| `/usr/local` | | apt **밖에서 관리자가 직접** 설치한 것. apt가 절대 건드리지 않아 충돌이 없다 — 우리 배포 스크립트가 여기 사는 이유 |
| `/opt` | optional | 자기 폴더 하나를 통째로 쓰는 서드파티 대형 패키지 (SQL Server가 `/opt/mssql`) — Windows 방식과 비슷하게 사는 예외 지대 |
| `/var` | variable | **돌면서 변하는 데이터**. 로그(`/var/log`)·DB 파일·웹 앱(`/var/www`)·캐시 |
| `/tmp` | temporary | **임시**. 재부팅에 사라져도 되는 것만. scp 착지점으로 쓰고 `install`로 제자리 이동 |
| `/home` | | 사용자별 홈. SSH 로그인 시작점 (`/home/ubuntu`) — 프롬프트의 `~`가 여기다 |
| `/root` | | root 계정의 홈 (`/home` 밖에 따로 있다) |
| `/bin` `/sbin` | binary | 부팅·복구에 필요한 핵심 명령. 요즘은 대부분 `/usr/bin`으로 통합(심볼릭 링크) |
| `/dev` `/proc` `/sys` | device·process | 실제 파일이 아니라 **커널이 보여주는 창** — 장치·프로세스 상태가 파일처럼 보인다 (`/proc/cpuinfo`를 cat해 보라) |

### 판단 축 세 개 — "어디 있을까/어디 놓을까"가 떠오를 때

1. **프로그램인가, 설정인가, 데이터인가** → 프로그램 `/usr` · 설정 `/etc` · 데이터/로그 `/var`
2. **누가 관리하나** → apt가 설치 `/usr/bin` · 내가 직접 `/usr/local/bin` · 서드파티 통짜 `/opt`
3. **변하는가** → 안 변함(실행 파일) `/usr` · 계속 변함(로그·DB) `/var` · 곧 사라져도 됨 `/tmp`

우리 서버에서 이 지도가 실제로 어떻게 채워졌는지(playground가 어디어디 사는지)는
`ServerLayoutAndLogs.md` §1의 표를 본다.

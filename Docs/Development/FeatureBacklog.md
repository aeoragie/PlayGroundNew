...
| 결제 | 회비·구독 등 결제 플로우 | **골격 착수(2026-08-13)** — PG는 토스페이먼츠 단일, `IPaymentGateway` 포트 + `Payments` 원장(종목 공통, `Sport` 컬럼 구분) + 테스트 결제 화면 `/dev/payments`(DEBUG 전용 flag). 남은 것: 웹훅 수신·취소/환불·정기결제(빌링)·실상품 연결(무엇을 팔지 미정). 팀 회비(`MonthlyFee`) 컬럼은 여전히 표시 전용 |

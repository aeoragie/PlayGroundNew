# PlayGround 개발 이력 (DevelopmentJournal)

> **2026-08-01, CLAUDE.md에서 이관.** 원래 CLAUDE.md의 "진행 상황 · 재개 가이드" 섹션이
> 페이즈별 작업 일지로 1200줄 넘게 커져, 매 세션 로드되는 CLAUDE.md를 가볍게 하려고 상세
> 이력을 이 문서로 분리했다. **내용은 이관 시점 그대로 보존**한다(정수 요약·현재 상태·반복
> 함정·미해결은 CLAUDE.md에 압축 유지).
>
> 성격: 각 기능이 **무엇을·왜 그렇게** 구현됐는지(설계 근거·경계 판단·겪은 함정·검증 결과·
> 남긴 미해결)의 시간순 기록. 특정 화면/기능의 배경을 알아야 할 때 여기서 검색한다.
> 날짜·커밋 해시는 이관 시점 기준이며, 이후 변경은 git 히스토리를 참조.
...
### TooltipHelp ⓘ 인포 팝오버 완료 (2026-07-21, Design.TooltipHelp §2 — 카탈로그 소비 0건이던 마지막 패턴)

- **`InfoPopover` 공용**(`Components/Shared/`) + `Css.Popover.cs` — ⓘ 20px 회색 원, 탭 토글+바깥 탭
  닫힘(투명 배경 — JS 없음), 흰 카드 radius 12·max-w 300px, 제목+본문 2~3문장+"자세히 보기" 링크
  슬롯(갈 화면 있을 때만 — 현재 미사용). **`OpenUp` 파라미터** — 푸터 등 하단은 위로+우측 정렬
  (아래로 열면 뷰포트 밖 — 실제 겪음). 호버 툴팁·첫 사용 힌트(§1·§3)는 미소비 유지(필요 화면 등장 시).
- **부착 4개념/5곳** (본문 카피는 dc에 없어 실용 작성, 열람 승인만 dc 데모 원문): ① 에이전트 심사
  "승인 시 열람되는 정보" — "에이전트 열람이란?" ② Claim 스텝 ② — "자녀 연결이란?" ③ 공개홈 히어로
  인증팀 — "인증팀이란?"(직접 붙일 수 없음 고지) ④ Records 목록·상세 푸터 — "기록은 어떻게
  집계되나요?"(설계 결정 7 요약, OpenUp).
- 검증(`shot-tooltip.js` 5 PASS): 토글·위로 열림 좌표·바깥 닫힘 왕복·Claim 스텝 ② 실진입(Pending
  코드) — 데이터 무변경. **에이전트 심사 ⓘ는 flag off라 화면 미확인**(코드 부착됨 — 다음 flag-on
  검증 때). 헤드리스 팁: Claim 코드 입력은 `locator.fill('input[aria-label="초대코드"]')` —
  value 대입+이벤트 발화로는 Blazor 바인딩이 안 잡힌다.

### 공개 팀 홈 리뷰 탭 완료 (2026-07-21, Design.TeamPublicHome §⑥) → **공개 팀 홈 6탭 전부 완성**

- **`SoccerTeamReviews` 신설** — 별점(1~5)·본문·작성 보호자·재원 근거 자녀·소프트 삭제. SP 3종
  (BySlug 공개+뷰어 상태 2결과셋 · Save 작성/수정 겸용 · Delete **작성자 본인만** — 팀 관리자 삭제
  경로 자체가 없다 = "팀은 삭제할 수 없고" 캡션 규칙).
- **재원 판정이 본체** — Save 프로시저가 판정: 보호자 연결(FamilyLinks Guardian) 자녀가 그 팀
  Active 소속. 자격 없음·중복·남의 리뷰 = 빈 결과 → Forbidden(사유 미구분). **계정당 팀 1건** —
  이미 쓴 계정은 신규 거부(수정 경로로).
- **서버 파생**: "김○○ 학부모"(**리뷰에도 실명 미노출** — dc '○' 마스킹) · 메타 "U15 · 재원 N년차"
  (팀 소속 시작일 기준) · "재원 확인됨" 뱃지는 작성 시 검증되므로 전 리뷰 표시. record의 CreatedAt
  자리를 재원 시작일로 씀(리뷰 작성일은 dc 카드에 표시가 없다 — @join 별칭 한계 회피).
- **공개 탭** `/team/{slug}/review` — 평균·개수는 **목록에서 클라 계산**(0건이면 평균 블록 미노출),
  캡션 원문. 반응형 단일 컴포넌트(`PublicReviewSection`).
- **작성 플로우 판단** (dc에는 열람만): 작성이 없으면 리뷰가 영원히 0건 → 서버 자격
  (IsResidentGuardian+미작성)일 때만 "리뷰 쓰기" 네이비 버튼 + 폼(별 5개 탭+본문 500자,
  `ReviewFormDialog`). 내 리뷰 카드 ⋯(수정/삭제+실행취소). **팀 답글 미구현** — dc 렌더 명세가
  없어 컬럼도 안 둠(지시 오면 마이그레이션과 함께). 리뷰 아바타는 dc 회색 원 대신 **개인 teal
  Avatar**(AvatarBadge 카탈로그 우선).
- 검증(`shot-reviewtab.js` 17 PASS): 재원 판정·마스킹·계정당 1건·무자격/경계/남의 리뷰 거부·
  수정→삭제→복구·폼 왕복(별 3점 선택→★★★☆☆ 반영)·모바일. 전량 원복.

### 공개 팀 홈 진학·진로 탭 완료 (2026-07-21, Design.TeamPublicHome §⑤ — 잔여 탭 리뷰 하나)

- **`SoccerTeamCareerOutcomes` 신설** (모집 공고와 같은 구조) — 연도·유형(ProTransfer/SchoolTeam/
  Promotion enum 문자열)·제목·상세·**PlayerCount**·소프트 삭제. 프로시저 4종(BySlug·ByManager·
  Save 신규/수정 겸용·Delete @Restore). 서버 검증: 유형 enum 화이트리스트·연도 1990~내년·인원 1~99·
  남의 사례 = 빈 결과 → Forbidden.
- **요약 3카드는 파생** — 사례 PlayerCount 유형별 합산(타임라인과 어긋날 수 없다). **값 있는 유형만
  카드 렌더**(빈 데이터 노출 금지). "동의한 사례만" 규칙은 동의 데이터가 없어 **입력 안내 카피로
  강제**(카드·폼 부제).
- **공개 탭** `/team/{slug}/career` — 요약 카드(모바일 3열·"상급팀 승격" 축약) → 타임라인(유형 태그
  = 프로 산하 오렌지톤/축구부 네이비톤/승격 teal톤, AvatarBadge 캡슐 통일) → 캡션 원문. 지연 로드,
  0건은 CTA 없는 빈 상태 문구. 반응형 단일 컴포넌트(`PublicCareerSection` — PC/모바일 분리 안 함).
- **관리자 편집 = 팀 정보 섹션 하단 관리 카드**(`CareerOutcomeManageCard`) — 편집 UI 명세가 SPEC에
  없어서 모집 탭 전례(공개홈 데이터는 대시보드에서 관리)를 따르되 사이드바에 자리가 없어 팀 정보에
  부속(B6 "신청이 시작된 자리" 판단과 같은 논리). 폼 다이얼로그(RadioCards 3유형·연도 프리필),
  삭제 = 확인 모달→소프트 삭제→실행취소(B3). **PC·모바일 두 트리가 카드를 각각 렌더하므로 데이터는
  페이지가 1회 로드**해 내려준다(카드 자체 로드면 API 2회 — 실제로 겪고 수정).
- 검증(`shot-careertab.js` 16 PASS): 저장→공개 즉시 반영(연도 역순)·경계 3종+남의 계정 거부·
  수정→삭제→복구 왕복·폼 빈 제출 인라인·태그 톤 계산값·요약 파생·빈 팀·모바일. 전량 원복.
...
- **미해결**: 증빙 사진 첨부(P1) · 상태 변경 알림은 기존 지연 생성 패턴을 타므로 별도 작업 없음 ·
  친선 거부는 프로시저 구조상 자명(팀 B6에서 별도 검증).
- **허브 자녀 카드 요약 추가 (2026-07-25, Design.RecordCorrection E2 잔여 — README "허브 자녀 카드" 배치 지시)**:
  전체 목록은 시즌 통계 아래 그대로 두고, 허브 자녀 카드(Claimed)에 **접수 건 수 요약+링크만** 얹었다.
  허브 커맨드가 보호자 corrections(RequestedByUserId 스코프, 기존 `GetRecordCorrectionsByManagerAsync` 재사용)의
  Pending MatchId를 자녀 시즌 스탯 MatchId와 대조해 `HubChildDto.CorrectionPendingCount` 산출(보호자 대시보드
  목록과 같은 MatchId 기준 — 수가 어긋나지 않는다). 0이면 미노출, >0이면 "기록 수정 신청 N건 처리 중 → 보기"
  (오렌지 톤, 링크 = 그 자녀 시즌 통계). **E2 나머지는 B6+보호자 경로에서 이미 완료.**
...
5. **Phase C — 신규 화면 전부 완료**: ~~허브~~ ~~팀 탐색~~ ~~설정~~ ~~Claim 4스텝·알림 센터~~ ~~모집 탭~~
   ~~에이전트 열람 승인~~(feature flag off) ~~진입점 연결·라우트 재감사~~ ~~진학·진로 탭~~ ~~리뷰 탭~~

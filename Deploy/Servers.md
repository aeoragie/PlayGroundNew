...
| 이미지 버킷 (S3) | `playgroundsport-images-599615474479-ap-northeast-2-an` (퍼블릭 차단 · 앱이 IAM 역할로 읽고 쓰며 `/uploads` 프록시로 서빙 — env `UploadStorageConfiguration__Provider=S3` + `__BucketName`) |

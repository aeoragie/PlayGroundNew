...
## 지금 어디까지 왔나 (2026-08-09 갱신)

**첫 배포 완료 — https://playgroundsport.com 라이브.** 스키마(계약 테스트 252건 통과)·
앱(수동 배포)·HTTPS(certbot, 자동 갱신 타이머)까지 끝났다.

> **인스턴스는 2026-08-09에 재생성됐다** (`i-001e9dbfc3b767c1f`, 같은 EIP 재연결).
> 8/3 서버에 했던 설치가 일부 비어 있었던 이유다(dotnet 등). SSH 호스트 키가 바뀌었으므로
> 접속 오류가 나면 `ssh-keygen -R 54.180.64.167` 후 재시도.

**운영 방침 (2026-08-09 결정):**
- **배포는 당분간 수동** — publish → tar zip → scp → `playground-deploy`. GitHub Actions
  시크릿 등록은 수동 절차가 손에 익은 뒤에 한다 (자산은 준비돼 있음 — README 4-5).
- **DB 백업 크론 보류** — 테스트 데이터뿐이라. **실데이터가 들어가기 전에 반드시 켠다**
  (README 4-4). 비번 회전도 그때 함께 (현재 비번은 대화·스크린샷에 노출된 적 있음).

### 이전 기록 (2026-08-03 셋업 당시)

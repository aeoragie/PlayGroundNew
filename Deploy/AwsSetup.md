...
1. **EC2** → **인스턴스** → **인스턴스 시작**
2. **이름**: `playground-prod`
3. **AMI**: **`모든 AMI 찾아보기`** → 검색창에 **`ubuntu-jammy-22.04`**
   - 고를 것: **`ubuntu/images/hvm-ssd/ubuntu-jammy-22.04-amd64-server-*`** · 소유자 **Canonical**(`099720109477`)
   - 아키텍처 **64비트(x86)**

   > ### 빠른 시작 목록의 `Ubuntu`를 그냥 고르면 안 된다
   >
   > 그 자리는 **항상 최신 LTS가 기본 선택**이라 26.04·24.04가 잡힌다.
   > **Microsoft는 26.04용 저장소를 제공하지 않고**, SQL Server 2022는 22.04까지다.
   > 그대로 진행하면 저장소 추가 단계에서 막혀 **SQL Server를 설치할 수 없다.**
   > 실제로 26.04로 한 번 만들어 다시 만들었다.
   >
   > **`jammy`가 22.04의 코드명이다.** 이 단어로 확인하는 것이 버전 숫자보다 확실하다.
   > 접속 후 첫 줄이 `Welcome to Ubuntu 22.04 LTS`인지 반드시 눈으로 본다.
4. **인스턴스 유형**: `t3.medium`
5. **키 페어**: "키 페어 만들기" 절에서 만든 `playground-prod`
6. **네트워크 설정** — **오른쪽 위 `편집`을 반드시 누른다.** 그 전에는 VPC·서브넷이
   글자로만 보이고 바꿀 수 없다(보안 그룹만 바로 고를 수 있어서 다 된 것처럼 보인다).
   편집을 누르면 드롭다운으로 바뀐다:
   - **VPC**: `playground-vpc`
   - **서브넷**: `playground-subnet-public1-…` (**퍼블릭**이어야 한다 — private을 고르면 접속이 안 된다)
   - **퍼블릭 IP 자동 할당**: **활성화** (직접 만든 VPC의 서브넷은 기본이 **비활성화**다)
   - **방화벽(보안 그룹)**: **기존 보안 그룹 선택** → `playground-prod-sg`

   > ### 편집을 안 누르면 AWS가 서브넷을 마음대로 고른다
   >
   > 기본 VPC라면 "기본 서브넷"을 고르지만, **직접 만든 VPC에는 기본 서브넷이 없어서
   > 임의로 하나를 집는다.** 이름순으로 `private1`이 `public1`보다 앞서 **프라이빗이 잡히기 쉽다.**
   > 실제로 이렇게 한 번 만들어 접속이 안 됐고, 서브넷은 나중에 바꿀 수 없어 다시 만들어야 했다.
...
### 만들자마자 확인 — 사설 IP로 서브넷을 판별한다

인스턴스의 **사설 IPv4 주소**만 보면 어느 서브넷에 들어갔는지 바로 알 수 있다.
마법사가 만든 CIDR 배치가 정해져 있기 때문이다:

| 사설 IP가 이 범위면 | 서브넷 | 판정 |
|---|---|---|
| `10.0.0.x` ~ `10.0.15.x` | public1 (`10.0.0.0/20`) | ✅ 정상 |
| `10.0.16.x` ~ `10.0.31.x` | public2 (`10.0.16.0/20`) | ✅ 정상 |
| **`10.0.128.x` ~ `10.0.143.x`** | private1 (`10.0.128.0/20`) | ❌ **잘못됐다** |
| **`10.0.144.x` ~ `10.0.159.x`** | private2 (`10.0.144.0/20`) | ❌ **잘못됐다** |

> **프라이빗 서브넷에 만들면 탄력적 IP를 붙여도 접속되지 않는다.**
> 라우팅에 IGW가 없어 나가지도 들어오지도 못한다. 게다가 **user-data가 패키지를 못 받아
> 설치 자체가 실패한다** — 인스턴스 상태는 "실행 중"이라 정상처럼 보인다.
>
> **인스턴스의 서브넷은 나중에 바꿀 수 없다.** 종료하고 다시 만든다
> (아래 "자주 막히는 것 정리" 참조).

EC2 → 인스턴스 → **네트워킹** 탭에서 서브넷 이름(`…-public1-…`)으로도 확인된다.
...
| **Ubuntu가 22.04가 아니다** | AMI 빠른 시작은 최신 LTS가 기본이다 → `jammy`로 검색해 다시 만든다 |
| **EIP를 붙였는데도 SSH가 안 된다** | **사설 IP가 `10.0.128~159.x`면 프라이빗 서브넷이다** → 아래 |

### 프라이빗 서브넷에 만들어 버렸을 때

증상: 탄력적 IP를 연결했는데도 접속이 시간 초과되고, 인스턴스 상태는 "실행 중"이다.
**user-data도 실패했다** — 패키지를 받으러 나갈 길이 없었기 때문이다.

**인스턴스의 서브넷은 변경할 수 없다. 종료하고 다시 만든다.**
아직 아무 데이터도 없으니 이게 가장 빠르고 깨끗하다.

1. EC2 → 인스턴스 → **네트워킹** 탭에서 서브넷 이름 확인 (`…-private1-…`이면 확정)
2. **탄력적 IP는 그대로 둔다** — 인스턴스를 종료해도 IP는 유지된다
3. 인스턴스 → **인스턴스 상태** → **인스턴스 종료(Terminate)**
4. "EC2 인스턴스 시작" 절대로 다시 만들되 **서브넷 = `…-public1-…`**
5. 새 인스턴스에 **탄력적 IP를 다시 연결**한다

> **탄력적 IP를 연결하지 않은 채 두면 시간당 요금이 붙는다.** 다시 만들 시간이 없으면
> 일단 해제(release)하고, 새로 만들 때 다시 할당해도 된다(주소는 바뀐다).

> 서브넷의 라우팅 테이블을 IGW로 바꿔 "퍼블릭으로 승격"시키는 방법도 되긴 한다.
> 하지만 이름은 `private`인데 실제로는 퍼블릭인 상태가 남아 **나중에 반드시 헷갈린다.**
> 아무것도 안 올라간 지금은 다시 만드는 쪽이 낫다.
